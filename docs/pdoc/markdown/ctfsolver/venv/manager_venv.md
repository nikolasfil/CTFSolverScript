Module ctfsolver.venv.manager_venv
==================================

Classes
-------

`ManagerVenv(*args, **kwargs)`
:   ManagerFolder provides utilities for managing folder structures and files for CTF (Capture The Flag) challenges.
    ...
    * ctfsolver.managers.manager_folder.ManagerFolder
    ...
    `activate(self, filepath)`
    :

    `check_global_venv_dir(self)`
    :

    `check_venv_dir(self, filepath)`
    :
    ...
    `delete_venv(self, folder: pathlib._local.Path)`
    ...
    `install_pipdeptree(self)`
    :

    `look_for_venvs(self, filepath: str = None)`
    ...
    `move_venv(self, folder: pathlib._local.Path = None)`
    :

    `save_dependencies(self)`
    :

    `system_translator(self)`
    :
